...
- **테스트 기간 동안 반영된 부분**
    - **세이브·체크포인트 :** 게임을 저장할 수 있는 오브젝트가 배치되었다.
      
    - **약간의 가이드 :** 맵상으로 문이 열렸는지 닫혔는지 확인할 수 있거나, 좀 더 직관적으로 배치하는 등으로 개선이 이루어졌다.
      
    - **상호작용 텍스트 :** 일부나마 상호작용 실패 시 텍스트를 출력시켜 의도된 사항임을 알리려 했다.

- **앞으로 고쳐나갈 부분**
    - **코드 최적화 :** 아직 최적화할 수 있는 코드들이 많이 남아 있다.
      
    - **상호작용 이펙트 :** 상호작용 + 상호작용 실패 시 적절한 효과음과 텍스트가 나타나 의도된 사항임을 알리고, 상호작용으로 무엇이 일어났는지 플레이어에게 전달해야 한다.
      
    - **튜토리얼/가이드 :** 게임 초반에 튜토리얼 세션을 만들어 해당 이벤트가 최초로 발생했을 때 튜토리얼 팝업과 UI 강조 등을 통해 핵심 기능을 안내하는 기능이 필요함.
      
    - **스토리 전달 :** 왜 이곳에 떨어졌는지, 뭘 해야 하는지에 대한 서사 전달이 필요하다. 미로에 들어오기 전의 인트로 씬이 있어야 하지만 아직은 생략된 상태.
      
    - **괴물 AI 개선 :** 현재는 괴물이 시야각에 들어와야만 추적을 시작하고 범위를 벗어나면 바로 추적이 멈추는 간단한 형태이기 때문에 입체적인 공포게임을 위해 좀 더 조정할 필요가 있다.
      
    - **탐지 기능의 당위성 부여 :** 기본적으로 배터리, 열쇠, 벽돌 등을 찾기 위해 탐지를 진행해야 한다는 기획이었으나, 탐지 가능한 대부분이 잡동사니이고 그중에 괴물이 껴있어 그냥 무시하더라도 별다른 패널티가 없다. 괴물의 경우 무시하면 더 무섭게 쫓아온다거나 꾸준히 아이템을 더 찾아야 하는 이유를 부여할 필요가 있다.
...
### 해결 방안
- **에셋 최적화:**
  - 에셋 텍스처 사이즈를 4096에서 512로 축소.
  - LOD Group을 사용해 카메라에 부담 감소.
  
- **라이트 최적화:** Light를 `Realtime`에서 `베이킹`으로 전환.
  
- **맵 최적화:** 맵 에디터에서 Room이 중복 저장되는 문제 수정.
  
- **카메라 최적화:** 
  - 사용 카메라 수를 줄임.
  - 카메라의 찍는 범위를 조정하고, 오클루전 컬링을 적용(10프레임 증가).
...
### 문제 상황
빌드 후 실행 시 로딩바가 시간이 지나도 차오르지 않는 문제가 있었습니다.

### 해결 방안
- **비동기 순서 문제 수정:**
  - `ObjectManager.Add`가 메인 스레드에서 실행되지 않아 문제가 발생.
    
  - Unity의 `AsyncOperation`과 상호작용 시, 메인 스레드에서 안전하게 처리되도록 수정.
  
  - `op.allowSceneActivation`이나 `op.progress` 확인 시 비동기 작업 완료를 보장하도록 순서를 재정렬.
...
### 문제 상황
길 안내 객체가 감지될 때 재생되는 사운드가, 객체가 꺼지고 자리로 돌아왔을 때 한 번 더 재생되는 문제가 있었습니다.
...
- **수정 결과:**
  - 사운드 재생 후 컨트롤러에서 상태 변경이 이루어지도록 조정하여 문제 해결.
...
### 문제 상황
스태미나가 모두 소진된 후에도 의도와는 다르게 스태미나를 즉시 사용할 수 있어 계속 달릴 수 있는 문제가 있었습니다.

### 해결 방안
- **조건문 수정:**
  - `isCrouched` 조건에서 웅크리지 않은 상태일 때 `enableSprint`가 항상 `true`로 설정되는 문제를 수정.
  - 달리는 중 웅크리기 버튼을 눌렀을 경우 `Return` 처리로 `enableSprint`가 갱신되지 않도록 변경.
    
- **외부 함수 호출 제거:**
  - 외부 함수에서 `enableSprint`를 강제 설정하던 부분을 삭제하여 문제를 해결.
