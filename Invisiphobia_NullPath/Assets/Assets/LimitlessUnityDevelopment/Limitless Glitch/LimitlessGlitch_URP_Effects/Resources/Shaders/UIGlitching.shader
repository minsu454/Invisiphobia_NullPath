Shader "LimitlessGlitch/UIGlitching" 
{
    Properties
    {
        _MainTex("Texture", 2D) = "white" {}
        _Mask("Mask", 2D) = "white" {}
        _FadeMultiplier("Fade Multiplier", Range(0,1)) = 0.0
        _TimeX("TimeX", Float) = 0.0
         _GlitchOffset("Glitch Offset", Float) = 0.5
    ...
    HLSLINCLUDE
        ...
        float4 Frag(Varyings i) : SV_Target
        {
            ...
            float4 orig = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv);
            float4 col = float4(yuv2rgb(sample_yuv), sample.a);

            // 최종 알파는 MainTex의 알파 또는 _FadeMultiplier 등의 값을 조합할 수 있음
            // 여기서는 sample.a를 사용하지만, 원한다면 col.a = col.a * _FadeMultiplier; 등으로 조정 가능
            float4 finalColor = lerp(orig, col, col.a);
            return finalColor;
        }
